...
const CourseContentSection = () => {
  ...
  const handleContentsState = (index: number) => {
    setContentsState((prev) => ({ ...prev, [index]: !contentsState[index] }));
  };
        ...
        {fields.map((field, index) => (
          ...
          >
            <div className="flex w-full justify-between shrink-0 items-center">
              <Subtitle2>{watch(`contents.${index}.video_section`)}</Subtitle2>
            ...
            <div
              className={cn(
                ...
                { "h-0 overflow-hidden opacity-0": !contentsState[index] }
              )}
            >
              <div>
                <InputWithLabel
                  ...
                  placeholder="Demo"
                  register={register}
                  error={errors?.contents?.[index]?.video_section}
                  required
                />
              </div>
              <div>
                <InputWithLabel
                  ...
                  register={register}
                  error={errors?.contents?.[index]?.video_title}
                  required
                />
              </div>
              ...
              <div className="col-span-full">
                <TextareaWithLabel
                  ...
                  error={errors?.contents?.[index]?.video_description}
                  required
                />
              </div>
              <div>
              ...
              <div>
                <InputWithLabel
                  ...
                  register={register}
                  error={errors?.contents?.[index]?.video_link_title}
                />
              </div>
              <div>
                <InputWithLabel
                  ...
                  placeholder="https://example.com"
                  register={register}
                  error={errors?.contents?.[index]?.video_link_url}
                />
              </div>
            </div>
          </div>
